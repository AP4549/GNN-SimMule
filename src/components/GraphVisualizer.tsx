...
export default function GraphVisualizer({ scenario, onAnalysis }: GraphVisualizerProps) {
  ...
  return (
      ...
      <div className="absolute top-6 left-6 z-10 flex flex-col gap-3">
        <div className="bg-card/90 backdrop-blur-sm p-5 flex flex-col gap-4 rounded-2xl border border-border soft-shadow">
          <div className="flex items-center gap-6">
            <div className="flex flex-col gap-1.5">
              <span className="text-[11px] font-semibold text-muted-foreground tracking-wide">Weight</span>
              <div className="flex items-center gap-3">
                <input type="range" min="0.1" max="1.0" step="0.1" value={weight}
                  onChange={e => setWeight(parseFloat(e.target.value))}
                  className="w-24 h-1.5 accent-primary bg-muted rounded-full appearance-none cursor-pointer" />
                <span className="text-xs font-mono text-foreground font-semibold w-6">{weight.toFixed(1)}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[11px] font-semibold text-muted-foreground tracking-wide">Aggregation</span>
              <div className="flex gap-1 p-1 bg-muted rounded-full">
                {(['SUM', 'MAX', 'MEAN'] as const).map(a => (
                  <button key={a} onClick={() => setAggregation(a)}
                    className={`px-3 py-1 text-[10px] font-semibold rounded-full transition-colors ${aggregation === a ? 'bg-primary text-primary-foreground soft-shadow' : 'bg-transparent text-muted-foreground hover:text-foreground'}`}>
                    {a}
                  </button>
                ))}
              </div>
        ...
        {/* Phase Indicator */}
        <div className="bg-card/90 backdrop-blur-sm px-4 py-2.5 flex items-center gap-3 w-fit rounded-full border border-border soft-shadow">
          <div className={`w-2.5 h-2.5 rounded-full ${isAnimating ? 'bg-primary animate-pulse' : 'bg-muted-foreground/50'}`} />
          <span className="text-xs font-medium text-foreground">{phaseLabel[phase]}</span>
          <div className="w-px h-3 bg-border"></div>
          <span className="text-xs font-mono text-muted-foreground font-semibold">L{layer}</span>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex items-center gap-4">
        <button onClick={handleReset}
          className="p-3.5 bg-card text-foreground rounded-full border border-border hover:bg-muted transition-colors group active:scale-95 soft-shadow">
        ...
        <button onClick={runNextPhase} disabled={isAnimating}
          className={`flex items-center gap-3 px-9 py-3.5 font-semibold text-sm rounded-full transition-colors relative overflow-hidden group soft-shadow ${
            isAnimating 
              ? 'bg-muted text-muted-foreground cursor-not-allowed' 
              : 'bg-primary text-primary-foreground hover:bg-primary/90 active:scale-95'
          }`}>
          <span className="relative z-10">{isAnimating ? 'Running…' : (phase === 'IDLE' ? 'Run analysis' : 'Next step')}</span>
          {!isAnimating && (
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="relative z-10"><path d="m9 18 6-6-6-6"/></svg>
          )}
        </button>

        <button onClick={toggleAutoPlay}
          className={`px-7 py-3.5 font-semibold text-xs rounded-full transition-colors active:scale-95 soft-shadow ${
            isAutoPlaying 
